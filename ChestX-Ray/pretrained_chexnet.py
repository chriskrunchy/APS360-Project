...
parser = argparse.ArgumentParser(description="Train a CheXNet model on chest X-ray images")
...
checkpoint = torch.load('/home/adam/final_project/APS360-Project/ChestX-Ray/chexnet_weights.pth')

# Extract the model's state_dict from the checkpoint
state_dict = checkpoint['state_dict']

# Remove 'module.' prefix if it exists (necessary if model was saved using DataParallel)
...
# Load state dict into model with strict=False to ignore non-matching keys
...
plt.title('CheXNet Receiver Operating Characteristic')
plt.legend(loc='lower right')
plt.savefig('chexnet_roc_curve.pdf')
